...
const App: React.FC = () => {
  ...
  const getStored = (key: string) => typeof window !== 'undefined' ? localStorage.getItem(key) : null;

  const [userId, setUserId] = useState<string>(getStored('dm_user_id') || '');
  const [partnerId, setPartnerId] = useState<string>(getStored('dm_partner_id') || '');
  const [userName, setUserName] = useState<string>(getStored('dm_user_name') || 'Ich');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Wenn DB leer ist -> Mit Mock-Daten füllen
        if (!recipesData || recipesData.length === 0) {
          console.log("Datenbank leer, starte Seeding...");
          const seedData = INITIAL_RECIPES.map(r => ({
            title: r.title,
            content: r.description,
            image_url: r.imageUrl,
            created_by: userId || 'system'
          }));
          const { data: seeded } = await supabase.from('recipes').insert(seedData).select();
          if (seeded) {
            setAllRecipes(seeded.map(r => ({
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#fdfcfb]">
        <div className="text-center">
          ...
          <p className="text-gray-500 font-medium">DinnerMatch wird vorbereitet...</p>
        </div>
      </div>
    );
  }
          ...
          {view === 'home' && (
            <motion.div key="home" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 pt-4">
              <div className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-orange-100/50 border border-orange-50 relative overflow-hidden">
                ...
                {!hasSwipedToday ? (
                  <button onClick={() => setView('swipe')} disabled={!dailyRecipe} className="w-full bg-orange-600 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-orange-700 active:scale-95 transition-all">Vorschlag ansehen</button>
                ) : (
            ...
            <motion.div key="swipe" className="h-[65vh] relative mt-4">
              {hasSwipedToday ? (
                <div className="flex flex-col items-center justify-center h-full text-center space-y-4">
                  ...
                  <p className="text-gray-500 px-10">Du hast für heute bereits abgestimmt.</p>
                ...
                <div className="text-center p-10">Keine Rezepte geladen.</div>
              )}
            </motion.div>
          )}
          ...
          {view === 'profile' && (
            <motion.div key="profile" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 pt-4">
              <div className="bg-white rounded-[2.5rem] p-8 shadow-lg border border-gray-100">
                <h2 className="text-2xl font-bold text-gray-900 mb-6">Profil & Partner</h2>
                <form onSubmit={saveProfile} className="space-y-6">
                  ...
                  <div>
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Deine ID (Für den Partner)</label>
                  ...
                  <button type="submit" className="w-full bg-gray-900 text-white font-bold py-4 rounded-2xl shadow-xl active:scale-95 transition-all">Speichern</button>
                </form>
              </div>
            </motion.div>
          )}
      ...
      <AnimatePresence>
        {isAddingRecipe && (
          <motion.div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-[2px]">
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="bg-white w-full max-w-md rounded-t-[2.5rem] p-8 shadow-2xl overflow-y-auto max-h-[90vh]">
              ...
              <form onSubmit={async (e) => {
                ...
                <div onClick={() => fileInputRef.current?.click()} className="h-40 bg-gray-50 border-2 border-dashed rounded-2xl flex items-center justify-center overflow-hidden cursor-pointer">
                  {newRecipe.imageData ? <img src={newRecipe.imageData} className="w-full h-full object-cover" /> : <div className="text-center text-gray-400"><Upload className="mx-auto mb-2" /><span className="text-xs">Bild hochladen</span></div>}
                ...
                <input required value={newRecipe.title} onChange={e => setNewRecipe({...newRecipe, title: e.target.value})} placeholder="Name des Gerichts" className="w-full bg-gray-50 p-4 rounded-xl border-none outline-none" />
                <textarea required value={newRecipe.content} onChange={e => setNewRecipe({...newRecipe, content: e.target.value})} placeholder="Kurze Beschreibung" className="w-full bg-gray-50 p-4 rounded-xl border-none outline-none resize-none" rows={3} />
                <button type="submit" disabled={isLoading} className="w-full bg-orange-600 text-white py-4 rounded-2xl font-bold shadow-lg disabled:opacity-50">
                  {isLoading ? <Loader2 className="animate-spin mx-auto" /> : "Rezept speichern"}
                </button>
              </form>
            </motion.div>
          </motion.div>
